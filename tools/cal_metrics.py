...
class SegmentationMetric():
            ...
            if metric == 'IoU':
                ...
                for img_info,result in zip(self.img_infos,self.results): 
                    ...
                    if min(acc) < self.fn_threshod: # missed detection
                        cls_id = int(np.argmin(acc))
                        if area_label[cls_id] < self.min_size:
                            continue
                        class_path = os.path.join(fn_path,self.class_names[cls_id])
                        pred_img = self.putMask(image,pred_label,self.palette,cls_id)
                        gt_img = self.putMask(image,label,self.palette,cls_id)
    ...
    def process(self):
        ...
        for filename in pred_gen:
            ...
            image_path = os.path.join(self.img_path,filename+self.img_ext)
    ...
    def putMask(image,mask,palette,class_id,alpha=0.7): 
        ...
        if isinstance(class_id,int):
            class_id = [class_id]
        for id_ in class_id:
            result[mask==id_+1] = result[mask==id_+1]*(1-alpha)+np.array([[palette[id_]]])*alpha
        return result
